...
impl TTFont {
    ...
    pub fn new<R: Read>(mut r: R) -> Result<Self, FontError> {
        let mut data = vec![0u8; 0];
        r.read_to_end(&mut data)
            .map_err(|_| FontError::UnknownError)?;

        let font = rusttype::Font::try_from_vec(data).ok_or(FontError::UnknownError)?;
        Ok(TTFont { font })
    }

    pub fn render_simple(&self, text: &str, pixel_height: u32) -> Result<Image, FontError> {
